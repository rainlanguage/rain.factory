...
/// @notice Every expectation is recomputed inline from the pinned formula,
/// never read back from the library, so a change to the library's derivation
/// diverges from the oracle here and the test fails.
