...
mod test {  
    ...
    async fn test_replace_no_dis() { 
        ...
        let tx_hash = H256::from_str(&String::from("0xea76ed73832498c4293aa06aeca2899f2b5adca15d703b03690185ed829f3e71")).unwrap() ;  
        // Reading environment variables
        let mumbai_network = Mumbai::new(env::var("MUMBAI_RPC_URL").unwrap(), env::var("POLYGONSCAN_API_KEY").unwrap()) ; 
        let network: RainNetworks = RainNetworks::Mumbai(mumbai_network);  
   ...
   async fn test_replace_only_from_dis() { 
        ...
        let tx_hash = H256::from_str(&String::from("0xc215bf3dc7440687ca20e028158e58640eeaec72d6fe6738f6d07843835c2cde")).unwrap() ;   
        // Reading environment variables
        let mumbai_network = Mumbai::new(env::var("MUMBAI_RPC_URL").unwrap(), env::var("POLYGONSCAN_API_KEY").unwrap()) ; 
        let network: RainNetworks = RainNetworks::Mumbai(mumbai_network) ;   
        let tx_data = get_transaction_data(network, tx_hash).await.unwrap() ; 
    ...
    async fn test_replace_from_to_dis() { 
        ...
        let tx_hash = H256::from_str(&String::from("0xebacdb3971924c9bbd2257334d436b4590d3d98f54969f6f942d6bd7a68da80b")).unwrap() ;   
        ...
        let expexted_tx_hash = H256::from_str(&String::from("0xb0ae6ff12e9b810530e1b0844a448865cf4781950a90c99ba36f7f343e596717")).unwrap() ; 
    ...
    async fn test_replace_dis() { 
        ...
        let fuji_network = Fuji::new(env::var("FUJI_RPC_URL").unwrap(), env::var("SNOWTRACE_API_KEY").unwrap()) ; 
        let to_network: RainNetworks = RainNetworks::Fuji(fuji_network);
        let expexted_tx_hash = H256::from_str(&String::from("0x13b9895c7eb7311bbb22ef0a692b7b115c98c957514903e7c3a0e454e3389378")).unwrap() ;   
        let expected_tx_data = get_transaction_data(to_network, expexted_tx_hash).await.unwrap() ;  
